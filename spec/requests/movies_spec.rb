...
RSpec.describe 'Movies', type: :request do
  ...
  describe 'PUT #update' do
    ...
    context '更新される場合' do
      it 'リクエストが成功すること' do
        put movie_path(@movie),
            params: { movie: FactoryBot.attributes_for(:movie, country_id: @country.id, title: 'アップデートタイトル') }
        expect(response).to have_http_status(302)
      end
    ...
    context '登録が失敗する場合' do
      it 'タイトルが変更されないこと' do
        expect do
          put movie_path(@movie),
              params: { movie: FactoryBot.attributes_for(:movie, country_id: @country.id, title: '') }
