...
RSpec.describe 'Theaters', type: :request do
  ...
  describe 'POST #create' do
    ...
    context '登録が失敗する場合' do
      it '登録が失敗すること' do
        expect do
          post theaters_path,
               params: { theater: FactoryBot.attributes_for(:theater, country_id: @country.id, writer: '') }
  ...
  describe 'PUT #update' do
    ...
    context '更新される場合' do
      it 'リクエストが成功すること' do
        put theater_path(@theater),
            params: { theater: FactoryBot.attributes_for(:theater, country_id: @country.id, title: 'テストアップデートタイトル') }
        expect(response).to have_http_status(302)
      end
    ...
    context '登録が失敗する場合' do
      it '作品名が変更されないこと' do
        expect do
          put theater_path(@theater),
              params: { theater: FactoryBot.attributes_for(:theater, country_id: @country.id, title: '') }
