...
RSpec.describe "Stages", type: :request do
  ...
  describe "POST #create" do
    ...
    context '登録が失敗する場合' do
      ...
      it "エラーメッセージが表示されること" do
        post stages_path, params: {stage: FactoryBot.attributes_for(:stage, theater_id: "", place_id: @place.id), start_date: ""}
        follow_redirect!
  ...
  describe "PUT #update" do
    ...
    context '更新される場合' do
      ...
      it "リクエストが成功すること" do
        put stage_path(@stage), params: {stage: FactoryBot.attributes_for(:stage, theater_id: @theater.id, place_id: @place.id, company: "アップデートカンパニー")}
        expect(response).to have_http_status(302)
      end
    ...
    context '登録が失敗する場合' do
      ...
      it "団体名が変更されないこと" do
        expect do
          put stage_path(@stage), params: {stage: FactoryBot.attributes_for(:stage, theater_id: @theater.id, place_id: @place.id, company: "")}
