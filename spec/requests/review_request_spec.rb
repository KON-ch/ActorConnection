...
RSpec.describe 'Reviews', type: :request do
  ...
  describe 'POST #create' do
    context '戯曲にレビューする場合' do
      ...
      it 'リクエストが成功すること' do
        post theater_reviews_path(@theater),
             params: { review: FactoryBot.attributes_for(:review, theater_id: @theater.id,
                                                                  review_page: theater_path(@theater)) }
      ...
      it '登録が失敗すること' do
        expect do
          post theater_reviews_path(@theater),
               params: { review: FactoryBot.attributes_for(:review, theater_id: @theater.id, review_page: theater_path(@theater),
                                                                    rate: '') }
    ...
    context '映画にレビューする場合' do
      ...
      it 'リクエストが成功すること' do
        post movie_reviews_path(@movie),
             params: { review: FactoryBot.attributes_for(:review, movie_id: @movie.id,
                                                                  review_page: movie_path(@movie)) }
        expect(response).to have_http_status(302)
      end
      ...
      it '登録が失敗すること' do
        expect do
          post movie_reviews_path(@movie),
               params: { review: FactoryBot.attributes_for(:review, movie_id: @movie.id,
                                                                    review_page: movie_path(@movie), rate: '') }
        end.to_not change(Review, :count)
      end
    ...
    context '舞台にレビューする場合' do
      ...
      it 'リクエストが成功すること' do
        post stage_reviews_path(@stage),
             params: { review: FactoryBot.attributes_for(:review, stage_id: @stage.id,
                                                                  review_page: stage_path(@stage)) }
        expect(response).to have_http_status(302)
      end
      ...
      it '登録が失敗すること' do
        expect do
          post stage_reviews_path(@stage),
               params: { review: FactoryBot.attributes_for(:review, stage_id: @stage.id,
                                                                    review_page: stage_path(@stage), rate: '') }
        end.to_not change(Review, :count)
      end
  ...
  describe 'PUT #update' do
    ...
    context '更新される場合' do
      it 'リクエストが成功すること' do
        put review_path(@review),
            params: { review: FactoryBot.attributes_for(:review, review_page: root_path, content: 'テストアップデートレビュー') }
        expect(response).to have_http_status(302)
      end
    ...
    context '更新が失敗する場合' do
      it 'レビューが変更されないこと' do
        expect do
          put review_path(@review),
              params: { review: FactoryBot.attributes_for(:review, review_page: root_path, rate: '') }
